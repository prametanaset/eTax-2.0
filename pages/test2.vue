...
function buildMerchantPayloadFromStore(shop: ReturnType<typeof useShopFormStore>) {
...
<template>
  <div>
    ...
    <Dialog v-model:open="open">
      <DialogContent class="max-w-lg w-full max-h-[90dvh] bg-[hsl(var(--card))] overflow-y-auto">
        <div class="flex flex-col">
          ...
          <div class="flex items-center gap-2 my-3 mx-2">
            <span class="flex-1 h-1.5 rounded-full transition-all" :class="step===1?'bg-primary':'bg-muted-200 dark:bg-muted-700'"/>
            <span class="flex-1 h-1.5 rounded-full transition-all" :class="step===2?'bg-primary':'bg-muted-200 dark:bg-muted-700'"/>
            <span class="flex-1 h-1.5 rounded-full transition-all" :class="step===3?'bg-primary':'bg-muted-200 dark:bg-muted-700'"/>
          </div>

          <!-- Tabs manual -->
          <div class="flex gap-2 mb-4 px-2">
            <Button :variant="tab==='corporate'?'default':'outline'" class="flex-1" @click="tab='corporate'">นิติบุคคล</Button>
            <Button :variant="tab==='person'?'default':'outline'" class="flex-1" @click="tab='person'">บุคคลธรรมดา</Button>
          </div>

          <!-- Step 1: ข้อมูลทั่วไป -->
          <form v-if="step===1" @submit.prevent="shop.nextStep()" class="space-y-5 px-2">
            <template v-if="tab==='corporate'">
              <div>
                <Label for="storeName">ชื่อร้านค้า</Label>
                <Input id="storeName" v-model="storeName" required placeholder="ชื่อบริษัท / ร้านค้า" class="w-full" />
              </div>
              <div>
                <Label for="taxId">เลขประจำตัวผู้เสียภาษี</Label>
                <Input id="taxId" v-model="taxId" required maxlength="13" placeholder="13 หลัก" class="w-full" />
              </div>
              <div>
                <Label for="branchCode">เลขที่สาขา</Label>
                <Input id="branchCode" v-model="branchCode" maxlength="5" inputmode="numeric" placeholder="5 หลัก" class="w-full" />
              </div>
            </template>
            <template v-else>
              <div>
                <Label for="contactName">ชื่อผู้ติดต่อ</Label>
                <Input id="contactName" v-model="contactName" required placeholder="ชื่อ-นามสกุล" class="w-full" />
              </div>
              <div>
                <Label for="personalId">เลขบัตรประชาชน</Label>
                <Input id="personalId" v-model="personalId" required maxlength="13" placeholder="13 หลัก" class="w-full" />
              </div>
            </template>
          ...
          <!-- Step 2: ที่อยู่ -->
          <form v-else-if="step===2" @submit.prevent="shop.nextStep()" class="space-y-5 px-2">
            <div>
              <Label>ที่อยู่ <span class="text-red-500">*</span></Label>
              <Textarea v-model="address" required class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary resize-none" rows="2" maxlength="150" placeholder="บ้านเลขที่ หมู่ ซอย ถนน" />
            </div>
            <!-- Replace with your real component -->
            <div class="my-1"><BaseLocationPicker /></div>
            <div class="flex gap-2 mt-6">
              <Button @click.prevent="shop.prevStep()" variant="outline" class="flex-1">ย้อนกลับ</Button>
          ...
          <div v-else-if="step===3" class="space-y-5 px-2">
            ...
            <div class="flex gap-2 mt-6">
              <Button @click="shop.prevStep()" variant="outline" class="flex-1">ย้อนกลับ</Button>
