...
const errorProvince = computed(() => touchedProvince.value && !selectedProvince.value)
const errorDistrict = computed(() => touchedDistrict.value && !selectedDistrict.value)
const errorSubdistrict = computed(() => touchedSubdistrict.value && !selectedSubdistrict.value)
...
const labelProvince = computed(() =>
  provinces.value.find(p => p.Value === selectedProvince.value)?.Label || 'เลือกจังหวัด'
)
const labelDistrict = computed(() =>
  districts.value.find(d => d.Value === selectedDistrict.value)?.Label || 'เลือกอำเภอ'
)
const labelSubdistrict = computed(() =>
  subdistricts.value.find(s => s.Value === selectedSubdistrict.value)?.Label || 'เลือกตำบล'
)
</script>
...
<template>
  <div class="space-y-4 max-w-md">
    ...
    <div>
            ...
            <CommandList>
              ...
              <CommandGroup>
                <CommandItem
                  ...
                  :key="p.Value"
                  :value="p.Value"
                  @select="selectedProvince = p.Value; showProvince = false; touchedProvince = true"
                >
                  {{ p.Label }}
                </CommandItem>
              </CommandGroup>
      ...
      <div v-if="errorProvince" class="text-xs text-red-500 mt-1">กรุณาเลือกจังหวัด</div>
    </div>
    ...
    <div>
            ...
            <CommandList>
              ...
              <CommandGroup>
                <CommandItem
                  ...
                  :key="d.Value"
                  :value="d.Value"
                  @select="selectedDistrict = d.Value; showDistrict = false; touchedDistrict = true"
                >
                  {{ d.Label }}
                </CommandItem>
              </CommandGroup>
      ...
      <div v-if="errorDistrict" class="text-xs text-red-500 mt-1">กรุณาเลือกอำเภอ</div>
    </div>
    ...
    <div>
            ...
            <CommandList>
              ...
              <CommandGroup>
                <CommandItem
                  ...
                  :key="s.Value"
                  :value="s.Value"
                  @select="selectedSubdistrict = s.Value; showSubdistrict = false; touchedSubdistrict = true"
                >
                  {{ s.Label }}
                </CommandItem>
              </CommandGroup>
      ...
      <div v-if="errorSubdistrict" class="text-xs text-red-500 mt-1">กรุณาเลือกตำบล</div>
    </div>

    <!-- รหัสไปรษณีย์ -->
    <div>
      <Label class="mb-1 block text-sm font-medium">รหัสไปรษณีย์</Label>
      <Input
        v-model="zipCode"
