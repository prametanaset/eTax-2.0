<template>
  <div class="">
    ...
    <BaseDateRangePicker @date-value="handleSelectDate" />
  </div>
</template>
...
const handleSelectDate = (dateRange: DateRange) => {
  mailStore.date.start = `after:${dateRange.start?.year}/${dateRange.start?.month}/${dateRange.start?.day}`;
  mailStore.date.end = `before:${dateRange.end?.year}/${dateRange.end?.month}/${dateRange.end?.day}`;
};
