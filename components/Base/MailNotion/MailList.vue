...
import { Dot, Paperclip, Inbox } from "lucide-vue-next";
...
<template>
  <div class="relative">
    ...
    <div v-else>
      <!-- ------------------mobile layout-------------------- -->
      <div
        class="xl:hidden overflow-y-auto px-2 h-[calc(100dvh-3.5rem)] custom-scroll"
        @scroll="onScroll"
      >
        <div class="w-full">
          <TransitionGroup name="list" appear>
            <template v-for="(group, label) in groupedEmails" :key="label">
              <div v-if="group.length">
                <div
                  v-if="label !== 'Today'"
                  class="py-2 pt-5 text-sm font-bold text-primary-500 bg-muted"
                >
              ...
              <div
                v-for="item in group"
                :key="item.id"
                class="cursor-pointer hover:bg-accent relative py-2 flex gap-3 border-b"
                @click="mailStore.setSelectMail(item)"
                :class="{
                  'bg-muted-300 dark:bg-muted-800 ':
                    mailStore.selectMail?.data.id === item.id,
                  'bg-[hsl(var(--card))] rounded-sm overflow-hidden':
                    !item.read,
                }"
              >
                <div class="">
