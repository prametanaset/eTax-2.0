...
<template>
  <div class="space-y-4">
      ...
      <PopoverContent class="p-0 w-80" align="end">
        <Command>
          ...
          <CommandList>
            ...
            <CommandGroup>
              <CommandItem v-for="product in availableProducts" :key="product.id" @select="addProduct(product)" :value="product.id" class="flex items-center px-4 py-2 cursor-pointer">
    ...
    <!-- Header Row -->
    <div v-if="products.length" class="grid grid-cols-[1fr_80px_80px_100px_50px] gap-4 p-2 text-gray-500 font-medium border-b items-center">
    ...
    <!-- Product List -->
    <div v-for="product in products" :key="product.id" class="grid grid-cols-[1fr_80px_80px_90px_50px] gap-4 items-center p-4 border rounded-lg">
      <!-- Product Info -->
      ...
      <div class="w-16 mx-auto">
        <Input type="number" v-model="product.quantity" min="1" class="text-center" />
      </div>

      <!-- Discount Popover -->
      <Popover>
        <PopoverTrigger as-child>
          <Button variant="outline" class="w-20 text-center">
      ...
      <!-- Delete Button -->
      <Button variant="ghost" class="text-red-500 hover:text-red-600" @click="removeProduct(product.id)">
        <Trash class="w-5 h-5" />
      </Button>
    </div>
